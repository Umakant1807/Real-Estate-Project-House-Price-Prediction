...
# Column names of training and test dataset
names(hd_train)
names(hd_test)

# Difference which column name is not available on test dataset
setdiff(names(hd_train), names(hd_test))

# Check missing value in imported training and test dataset
sum(is.na(hd_train))
sum(is.na(hd_test))
...
# Combine both datasets into one for data cleaning purpose 
hd_all <- rbind(hd_train, hd_test)

# To check the combined dataset which names as hd_all
glimpse(hd_all)
...
write.csv(test.pred, "Umakant_Part_2.csv", row.names = F)
